...
# CHECK DIMENSIONS (VERIFICATION)
try:
    cbow_loaded = Word2Vec.load("cbow_model.model")
    print(f"CBOW vector size: {cbow_loaded.vector_size}")
except:
    print("CBOW model not found or error loading")

try:
    skipgram_loaded = Word2Vec.load("skipgram_model.model")
    print(f"Skipgram vector size: {skipgram_loaded.vector_size}")
except:
    print("Skipgram model not found or error loading")

# PRINT EMBEDDING VECTOR
word_to_print = "research"
if word_to_print in cbow_model.wv:
    vector = cbow_model.wv[word_to_print]
    vector_str = ", ".join([f"{x:.4f}" for x in vector])
    print(f"{word_to_print} - {vector_str}")
else:
    # Fallback
...
print("Top 30 frequent words (List):", freq.most_common(30))
...
for word, count in top_10:
    output_parts.append(f"{word}, {count}")
print("Top 10 Words (Formatted): " + ", ".join(output_parts))
...
# Specific requested analogies
analogies_to_try = [
    ("student", "learning", "professor"),
    ("student", "research", "professor"), 
    ("student", "research", "faculty"),
    ("image", "vision", "text"),
    ("study", "student", "teach"),
    ("study", "learning", "teach"),
    ("good", "better", "bad"),
    ("iit", "jodhpur", "institute"),
    ("computer", "science", "electrical")
]

for w1, w2, w3 in analogies_to_try:
    try:
        # Check vocab existence first to avoid hard crash if word is missing
        if (w1 in cbow_model.wv and w2 in cbow_model.wv and w3 in cbow_model.wv):
            res = cbow_model.wv.most_similar(positive=[w2, w3], negative=[w1], topn=1)
            if res:
                print(f"{w1} -> {w2} :: {w3} -> {res[0][0]} (score: {res[0][1]:.2f})")
        else:
            print(f"Skipping {w1}:{w2}::{w3} - Word(s) not in vocabulary")
            
    except Exception as e:
        print(f"Analogy error for {w1}, {w2}, {w3}: {e}")

# Original manual checks
try:
    print("Course:Grade :: Student:?", cbow_model.wv.most_similar(positive=["course", "grade"], negative=["student"], topn=1))
except: pass

try:
    print("Course:Registration :: Semester:?", cbow_model.wv.most_similar(positive=["course", "registration"], negative=["semester"], topn=1))
except: pass

try:
    print("Course:Project :: Research:?", cbow_model.wv.most_similar(positive=["course", "project"], negative=["research"], topn=1))
except: pass
